...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # check if the prerequisites are a subset of the previous literals level
            if a_node.prenodes.issubset(self.s_levels[level]):
                # add this action to the set of actions at this level
                actions.add(a_node)
    ...
    def add_literal_level(self, level):
        ...
        literals = set()
        # for each action in the previous action level
        for a_node in self.a_levels[level - 1]:
            # for each of the action's effect literals
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # definition: one action negates an effect of the other
        # we want to check which fluents are true, as added by node_a1, and also true, as removed by node_a2
        # i.e. the set intersection of node_a1's added fluents with node_a2's removed fluents
        added_but_removed = set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)
        # similar, but the intersection of node_a1's removed fluends with node_a2's added fluents
        removed_but_added = set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)
        # return the size of the set union of this, as we want to know if there is ANY such case
        # if there are such cases, we got an inconsistent effect mutex
        return len(added_but_removed | removed_but_added) > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # definition: one of the effects of one action is the negation of a precondition of the other
        # similar approach as we did for inconsistent_effects_mutex
        n1_added_but_neg = set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)
        n1_removed_but_pos = set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)
        n2_added_but_neg = set(node_a2.action.effect_add) & set(node_a1.action.precond_neg)
        n2_removed_but_pos = set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos)

        return len(n1_added_but_neg | n1_removed_but_pos | n2_added_but_neg | n2_removed_but_pos) > 0
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # definition: one of the preconditions of one action is mutually exclusive with a precondition of other
        # we will use the is_mutex check on each combination of preconditions
        for n1_precond in node_a1.parents:
            for n2_precond in node_a2.parents:
                if n1_precond.is_mutex(n2_precond):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for n1_action in node_s1.parents:
            for n2_action in node_s2.parents:
                if not n1_action.is_mutex(n2_action):
                    return False
        return True
